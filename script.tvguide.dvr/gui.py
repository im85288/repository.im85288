...
class TVGuide(xbmcgui.WindowXML):
    ...
    def __init__(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/script.tvguide.dvr/categories.ini','rb')
    ...
    def getControl(self, controlId):
        try:
            ...
            xbmc.log("EXCEPTION: (script.tvguide.dvr) TVGuide.getControl %s" % detail, xbmc.LOGERROR)
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() == ACTION_LEFT:
            ...
            xbmc.log('[script.tvguide.dvr] Unhandled ActionId: ' + str(action.getId()), xbmc.LOGDEBUG)
...
class PopupMenu(xbmcgui.WindowXMLDialog):
    ...
    def onAction(self, action):
        if action.getId() in [ACTION_PARENT_DIR, ACTION_PREVIOUS_MENU, KEY_NAV_BACK]:
            ...
            f = xbmcvfs.File('special://profile/addon_data/script.tvguide.dvr/categories.ini','wb')
...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
    ...
    def onClick(self, controlId):
        if controlId == self.C_STREAM_BROWSE_ADDONS:
            ...
            dialog = xbmcgui.Dialog()
            stream = dialog.browse(1, ADDON.getLocalizedString(30304), 'files', '.strm|.m3u|.m3u8')
            if stream:
                stream_name = stream
    ...
    def addBrowseFolder(self):
        file_name = 'special://profile/addon_data/script.tvguide.dvr/folders.list'
        f = xbmcvfs.File(file_name)
        items = f.read().splitlines()
        f.close()
        ...
        file_name = 'special://profile/addon_data/script.tvguide.dvr/icons.ini'
